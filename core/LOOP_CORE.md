...
- project-roadmap owns project decisions.
...
- In this collaboration exchange, Executor returns its Result directly to project-roadmap.
- When project-roadmap decides to Continue and another Task must be executed, it must produce a new PROJECT_EXECUTION_REQUEST for that successor Task.

## Stage

Stage is a Loop lifecycle concept. project-roadmap defines the Stage and Launcher maintains its lifecycle. Both know the Stage context. Executor receives only the current Task context; Stage information is provided only when strictly required for that Task, never as full Stage context.
...
```text
project-roadmap initialization
  ↓
Stage creation
  ↓
...
| --- | --- | --- |
| project-roadmap | Explicit role assignment and the applicable collaboration rules and Contract material. | Project context needed to define the project goal, Stage, Tasks, and acceptance and make project decisions. |
...
The LoopFeishu Notification Skill, when used, is an optional observability capability. It is not part of the Loop Contract, is not a collaboration message, does not own lifecycle control, and does not affect Task execution. Launcher may use it for lifecycle observability, and Executor may use it for execution observability. Notification failure must not prevent Task execution, TASK_EXECUTION_RESULT generation, or the Stage decision.
